...
Server architecture is currently monolithic. All the resources a server owns are
located within a single machine. In the common case these resources are a CPU,
DRAM, and some amount of persistent storage (disk/SSD). Within a data center,
servers are assigned widely differing roles such as data base managers,
key-value stores, and machine learning coordinators despite having similar if
not identical physical resources. Proponents of resource disaggregation contend
that monolithic servers waste resources due to poor bin packing, and are
inflexible to changing requirements. For a resource to be disaggregated it is
self-contained, and accessible over a network. Disaggregated disks for instance
have become commonplace within the last decade. The goal of complete physical
disaggregation is that every server resource (CPU, memory, storage) be placed in
a separate physical unit and be interconnected by a network.
Figure~\ref{fig:rackscale} is an illustration of rack-scale
disaggregation~\cite{disandapp}. Each server in the rack has a role, and all of
the servers are interconnected by a fast network with a top of rack (TOR) switch
at the center of the network. In disaggregated proposals memory and storage
servers are passive, meaning that they have no colocated CPUs and are accessed
exclusively by operations issued over the network by compute servers.
...
\begin{figure}[t]
    \begin{centering}
    \includegraphics[width=0.40\textwidth]{fig/rack.pdf}
...
Partitioning servers into distinct roles provides a high degree of resource
flexibility but comes at an additional latency cost. The access latency between
a CPU and its local memory is 20x faster than memory accessed over a network. In
pure resource disaggregation remote accesses are the norm, some proposals
suggest using the CPUs last level cache fetches and evictions trigger remote
reads and writes~\cite{legoos,kona}. This means that a naively implemented
disaggregated server will be orders of magnitude slower than a monolithic
server. Given that reads and writes from compute servers to memory servers must
be fast, disaggregation proposals typically leverage RDMA as a means to access
remote memory.  RDMA is a popular networking technology which allows clients to
read and write directly to a servers memory without CPU involvement. RDMA is
also known for having high throughput and low latency. Due to its direct
applicability most memory disaggregation proposals assume RDMA-capable networks
as an essential architectural building block. However, alternative interconnect
proposals exist.

Reading and writing to memory with RDMA is a substantially different interface
than the load/store path a CPU uses for local memory.  The RDMA specification
gives a set of \textit{verbs} which are memory operations that the NIC can
execute. These verbs are similar in some respects to RPC calls, or simple
...
% Research from RDMA key-value stores has shown that the cost of using one sided operations 
...
Loose constraints can enable operations to succeed even when they are executed
...
Any degree of global churn in a data structure, such as resizing a hash table
...
% on an RDMA network its going to be most of the traffic. For example the
...
% grow arbitrarily. Another technique is to make each entry have its own checksum
% for its integrity, and to only update the bucket checksum when all items become
% static.
...
%reduce the cost of reading. While this makes sense its not super applicable.
...
% management to the client side, and just let the RDMA do its thing with the
...
\sg{while reading I think I found a linearizability bug in RACE, I think its
...
We end by describing how RACE may benefit from in-network logic. RACE's search
...
% Deferring all locking operations to a switch for instance dominates its spare
...
%     \item its vs its. I know the difference, I think I just make this mistake
%     while I'm writing.

%     \item Alex notes, strikes mean remove, circles mean wtf, added punctuation
%     means add punctuation.

%     \item add serialization early in the document so that its listed as a key problem
...
% identical. its the same author, I'm just noting that the change between NUMA
...
%         & project & \shortstack{read \\ inflation} & \shortstack{relaxed  \\ layout} & \shortstack{pre-\\compute} & \shortstack{optimistic \\ concurrency} & \shortstack{metadata \\ caching} & \shortstack{compute \\ coalescing} & \shortstack{self \\ verifing}  \\ \hline
%                                                                 %N          %R          %P          %O          %M          %C          %W          %S
% %Multicore NUMA & \shortstack{Flat \\ Combining}~\cite{flat-combine}   & \nullcirc & \nullcirc & \nullcirc & \nullcirc & \nullcirc & \fullcirc & \nullcirc \\ \hline
% %Multicore NUMA & \shortstack{Hopscotch \\ Hash}~\cite{hopscotch}         & \fullcirc & \nullcirc & \nullcirc & \nullcirc & \nullcirc & \nullcirc & \nullcirc \\ \hline
% %Multicore NUMA & \shortstack{Blackbox \\ NUMA}~\cite{black-box-numa}     & \halfcirc & \nullcirc & \nullcirc & \fullcirc & \fullcirc & \fullcirc & \nullcirc \\ \hline %\cline{2-10} \cline{2-10}
% %\shortstack{RDMA \\ Key-Value} & pilaf~\cite{pilaf}                                      & \nullcirc & \nullcirc & \nullcirc & \nullcirc & \nullcirc & \nullcirc & \fullcirc \\ \hline
% %\shortstack{RDMA \\ Key-Value} & farm~\cite{farm}                                        & \fullcirc & \nullcirc & \nullcirc & \fullcirc & \fullcirc & \fullcirc  & \nullcirc \\ \hline
% %\shortstack{RDMA \\ Key-Value}& herd~\cite{herd}                                        & \nullcirc & \nullcirc & \nullcirc & \nullcirc & \nullcirc & \nullcirc  & \nullcirc \\ \hline
% %\shortstack{RDMA \\ Key-Value}& cell~\cite{cell}                                        & \nullcirc & \nullcirc & \nullcirc & \halfcirc & \fullcirc & \nullcirc  & \fullcirc \\ \hline
% %\shortstack{\small Disaggregated \\ \small Datastructures} & Clover~\cite{clover}                                    & \nullcirc &  \halfcirc &  \nullcirc & \fullcirc & \fullcirc  & \nullcirc & \nullcirc \\ \hline
% %\shortstack{\small Disaggregated \\ \small Datastructures} & \shortstack{RACE}~\cite{write-op-hash}            & \fullcirc  & \fullcirc & \halfcirc & \fullcirc & \fullcirc & \nullcirc & \fullcirc \\ \hline
% %\shortstack{\small Disaggregated \\ \small Datastructures} & \shortstack{Sherman \\ ~\cite{sherman}}                            & \fullcirc & \fullcirc & \halfcirc & \fullcirc & \fullcirc & \fullcirc & \fullcirc \\ \hline
% \shortstack{\small Disaggregated \\ \small Datastructures} & Mind~\cite{mind}                                              & \nullcirc & \halfcirc & \halfcirc & \nullcirc & \fullcirc & \nullcirc & \nullcirc \\ \hline
%     \end{tabular}
