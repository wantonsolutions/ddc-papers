...
def tput_err(ax,rws,ws, clover):
    ...
    scale_mil(ws)
    scale_mil(clover)
    ax.errorbar(rws["threads"],rws["ops"],rws["err"],label="Swordbox", marker=read_write_steering_marker, color=read_write_steering_color, linewidth=default_line_width, markersize=default_marker_size)
...
# ax3.set_title('Contention at 50% Writes')
...
# fig.text(0.515,0.00, "Threads", ha='center', size=14)
plt.tight_layout()
save_fig(plt)